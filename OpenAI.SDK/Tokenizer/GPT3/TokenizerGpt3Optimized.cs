...
///     GPT3 Tokenizer.
/// </summary>
public static class TokenizerGpt3Optimized
{
    ...
    private static IEnumerable<string> GetNewTokens(string text)
    {
        ...
        foreach (var match in matches.Cast<Match>())
        {
            ...
            var token = new StringBuilder(tokenBytes.Length);
            foreach (var b in tokenBytes)
            {
                token.Append(byteEncoder[b]);
            }
            var newTokens = BytePairEncoding(token.ToString()).Split(' ');
