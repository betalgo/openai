...
class CoreBPE
{
    ...
    public List<T> BytePairMerge<T>(byte[] piece, Dictionary<byte[], int> ranks, Func<ArraySegment<byte>, T> f)
    {
        ...
        {
            int? rank = getRank(piece, ranks, parts, i, 0);
            if (rank.HasValue)
            {
        ...
        while (parts.Count > 1)
        {
            ...
            if (minRank != int.MaxValue)
            {
                ...
                parts[i] = (parts[i].start, getRank(piece, ranks, parts, i, 1) ?? int.MaxValue);
                if (i > 0)
                {
                    parts[i - 1] = (parts[i - 1].start, getRank(piece, ranks, parts, i - 1, 1) ?? int.MaxValue);
                }
        ...
        for (int i = 0; i < parts.Count - 1; i++)
        {
            //output.Add(piece[parts[i].start..parts[i + 1].start]);
            output.Add(f(new ArraySegment<byte>(piece, parts[i].start, parts[i + 1].start - parts[i].start)));
        }

        return output;
    }

    private int? getRank(byte[] piece, Dictionary<byte[], int> ranks, List<(int start, int rank)> parts, int startIdx, int skip)
    {
        if (startIdx + skip + 2 < parts.Count)
        {
            //byte[] key = piece[parts[startIdx].start..parts[startIdx + skip + 2].start];
            byte[] key = new ArraySegment<byte>(piece, parts[startIdx].start, parts[startIdx + skip + 2].start - parts[startIdx].start).ToArray();         
    ...
    /// <param name="ranks"></param>
    /// <returns></returns>
    public List<int> BytePairEncode(byte[] piece, Dictionary<byte[], int> ranks)
    {
        if (piece.Length == 1)
        {
